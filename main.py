...
def bill_button_operation():
    ...
    if ans == "yes":
        ...
        st = "\t\t\t\tPRADEEP Hotel\n\t\t\tRam temple circle, Anekal-562106\n"
        st += "\t\t\tGST.NO:- 27AHXPP3379ZIHA\n"
        st += "-" * 61 + "BILL" + "-" * 61 + "\nDate:- "
        ...
        bill_number = random.randint(1000, 9999)  # Generate a random 4-digit bill number
        st += f"Bill Number:- {bill_number}\n"  # Add the bill number as an integer value
    ...
    # Generate a unique bill number using UUID
    bill_number = str(uuid.uuid4())
    

    # Hash the bill number using SHA-256
    hashed_bill_number = hashlib.sha256(bill_number.encode()).hexdigest()

    # Rest of the code...

    # Display the bill number
    st += "Bill Number: " + hashed_bill_number + "\n\n"
...
root.title("Welcome to PRADEEP HOTEL")
...
title_frame = Frame(root, bd=8, bg="black", relief=GROOVE)
title_frame.pack(side=TOP, fill="x")

title_label = Label(title_frame, text="PRADEEP HOTEL", 
                    font=("times new roman", 25, "bold"),bg = "black", fg="white", pady=5)
title_label.pack()
